...
Implement one task, verify it, update `PROGRESS.md`, and create one local commit.
...
12. Update target line in `PROGRESS.md` to `[x]`; add date, checks, and concise changed-file list. Recalculate counts.
13. Run `git diff --check` and inspect `git status --short`. Stage only task-owned files plus `PROGRESS.md`, then create one concise local commit.
14. Report only task number/title, changed files, check results, progress count, and commit hash. State that it is ready for GitHub PR review. Then stop.
...
Finish and commit one task before starting another.
