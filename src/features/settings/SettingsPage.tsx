    ...
    <main className="min-w-0 max-w-[70rem] p-[clamp(1.25rem,3vw,2.5rem)] [overflow-wrap:anywhere] max-[31rem]:px-3 max-[31rem]:py-4">
      ...
      <section className={panelClass} aria-labelledby="settings-clipboard">
        ...
        <div className="grid grid-cols-2 gap-3 max-[50rem]:grid-cols-1">
          <label className={labelClass}>History retention (days, 1-365)<input className={fieldClass} type="number" min={1} max={365} value={settings.history_days} disabled={busy} onChange={(event) => update("history_days", Number(event.target.value))} /></label>
          <label className={labelClass}>Maximum items (10-10,000)<input className={fieldClass} type="number" min={10} max={10000} value={settings.max_items} disabled={busy} onChange={(event) => update("max_items", Number(event.target.value))} /></label>
          <label className={labelClass}>Ignored apps<textarea className={fieldClass} value={settings.ignored_apps.join("\n")} disabled={busy} onChange={(event) => update("ignored_apps", event.target.value.split("\n").map((v) => v.trim()).filter(Boolean))} placeholder="One executable per line" /></label>
          <label className={labelClass}>Ignored text patterns<textarea className={fieldClass} value={settings.ignored_patterns.join("\n")} disabled={busy} onChange={(event) => update("ignored_patterns", event.target.value.split("\n").map((v) => v.trim()).filter(Boolean))} placeholder="One regular expression per line" /></label>
        </div>
      ...
      <section className={panelClass} aria-labelledby="settings-appearance">
        ...
        <label className={labelClass}>Formatter indent (spaces, 1-8)<input className={fieldClass} type="number" min={1} max={8} value={settings.formatter_indent} disabled={busy} onChange={(event) => update("formatter_indent", Number(event.target.value))} /></label>
      </section>
