  ...
  return (
    ...
    >
      ...
      <span
        aria-hidden="true"
        className={cn(
          "grid size-4 shrink-0 place-content-center rounded-sm border transition-colors peer-focus-visible:outline-2 peer-focus-visible:outline-offset-2 peer-focus-visible:outline-[var(--color-focus)]",
          checked
            ? "border-[var(--color-border-accent)] bg-primary"
            : "border-[var(--color-border-strong)] bg-[var(--color-surface-muted)]",
          !checked && !disabled && "group-hover:border-[var(--color-accent-dim)]",
        )}
      >
        <svg
          viewBox="0 0 16 16"
          className={cn(
            "size-3 fill-none stroke-[var(--color-accent-ink)] [stroke-linecap:round] [stroke-linejoin:round] [stroke-width:2.4] transition-transform",
