...
Each clipboard item row SHALL show a metadata line beneath its content containing the size or line count and any type-specific detail (e.g. dimensions for images, key count for JSON). Source application SHALL be included in this line only once source-app capture is persisted per item (tracked separately; not part of this change — see proposal Non-goals).
