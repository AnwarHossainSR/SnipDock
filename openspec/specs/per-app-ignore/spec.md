...
Capture SHALL continue to drop any capture whose foreground executable matches an entry in `Settings.ignored_apps`. The panel edits the contents of that list only; the capture-time filter that consults it behaves exactly as it did before the panel existed.

#### Scenario: A capture from an ignored app is dropped
- **WHEN** `ignored_apps` contains `Code.exe` and content is copied while `Code.exe` is the foreground application
- **THEN** no item is stored for that copy

#### Scenario: Removing an entry restores capture from that app
- **WHEN** `Code.exe` is removed from the list and content is copied from it afterwards
- **THEN** the capture is stored like any other
