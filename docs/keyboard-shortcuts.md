...
Only Quick Paste is a system-wide (global) shortcut. All other shortcuts work while the SnipDock window has focus, so they never block `Ctrl+Shift` shortcuts in other applications (VS Code's command palette, search, etc.). The selected-item shortcuts act on the Clipboard History page.

`CmdOrCtrl+K` opens the command palette in the main window, and pressing it again closes it. Type to filter its commands: pause or resume capture, save an item, switch between light and dark, open Settings, clear history, copy one of the three most recent captures, show only pinned captures, narrow to your busiest source app, or jump to Settings › Privacy. `Up`/`Down`, `Home`, and `End` move the selection, `Enter` runs it, and `Escape` closes the palette. Text that matches no command searches the clipboard history instead. The palette key is fixed rather than one of the rebindable bindings listed above; while the palette is open, the selected-item shortcuts are paused so they cannot act on the page behind it.

Every binding above is the default, not a fixed value: **Settings → Keyboard** rebinds any of them, and a rebind takes effect without a restart. The bullet list is also parsed at build time into the schema that panel renders and that the dispatch layer matches against, so a binding written here is the one the app registers — keep the `- Label: \`CmdOrCtrl+…\`` shape when editing.
